...
def fit_gaussian_knockoff_params(Xb_tr, eps=1e-12, cap_s=0.99, rng=None, shrinkage=True, alpha=0.1):
    if rng is None:
    ...
    Sigma = (Xc.T @ Xc) / (n - 1)
    if shrinkage:
        Sigma = (1-alpha) * Sigma + alpha * np.eye(p)
...
def make_train_knockoffs(X_tr, seed=1):
